@implementation MCColorNode

+(void *)nodeWithColor4B:(struct _ccColor4B)arg2 {
    memcpy(&r2, &arg2, 0x4);
    r0 = [MCColorNode alloc];
    r0 = [r0 initWithColor4B:r2 & 0xffffffff];
    r0 = [r0 autorelease];
    return r0;
}

+(void *)nodeWithColor4B:(struct _ccColor4B)arg2 width:(float)arg3 height:(float)arg4 {
    memcpy(&r2, &arg2, 0x4);
    r0 = [MCColorNode alloc];
    r0 = [r0 initWithColor4B:r2 & 0xffffffff width:r3 height:r4];
    r0 = [r0 autorelease];
    return r0;
}

+(void *)nodeWithColor4F:(struct _ccColor4F)arg2 width:(float)arg3 height:(float)arg4 {
    memcpy(&r2, &arg2, 0x8);
    r0 = [MCColorNode alloc];
    r0 = [r0 initWithColor4F:r2 width:r3 height:r4];
    r0 = [r0 autorelease];
    return r0;
}

+(void *)nodeWithColor4F:(struct _ccColor4F)arg2 {
    memcpy(&r2, &arg2, 0x8);
    r0 = [MCColorNode alloc];
    r0 = [r0 initWithColor4F:r2];
    r0 = [r0 autorelease];
    return r0;
}

+(void *)nodeWithColor3B:(struct _ccColorThreeB)arg2 {
    memcpy(&r2, &arg2, 0x4);
    r0 = [MCColorNode alloc];
    r0 = [r0 initWithColor3B:r2 & 0xffffffff];
    r0 = [r0 autorelease];
    return r0;
}

-(void *)init {
    r0 = [self initWithColor4B:0xffffffffffffffff width:r3 height:r4];
    return r0;
}

-(void *)initWithColor4F:(struct _ccColor4F)arg2 width:(float)arg3 height:(float)arg4 {
    memcpy(&r2, &arg2, 0x8);
    r0 = self;
    asm { fcvtzs     w2, s0 };
    asm { fcvtzs     w8, s0 };
    asm { fcvtzs     w9, s0 };
    asm { fcvtzs     w10, s0 };
    asm { bfi        x2, x9, #0x10, #0x8 };
    asm { bfi        x2, x10, #0x18, #0x8 };
    asm { bfi        x2, x8, #0x8, #0x8 };
    r0 = [r0 initWithColor4B:r2 width:r3 height:r4];
    return r0;
}

+(void *)nodeWithColor3B:(struct _ccColorThreeB)arg2 width:(float)arg3 height:(float)arg4 {
    memcpy(&r2, &arg2, 0x4);
    r0 = [MCColorNode alloc];
    r0 = [r0 initWithColor3B:r2 & 0xffffffff width:r3 height:r4];
    r0 = [r0 autorelease];
    return r0;
}

-(void *)initWithColor4F:(struct _ccColor4F)arg2 {
    memcpy(&r2, &arg2, 0x8);
    r0 = [self initWithColor4F:r2 width:r3 height:r4];
    return r0;
}

-(void *)initWithColor4B:(struct _ccColor4B)arg2 {
    memcpy(&r2, &arg2, 0x4);
    r0 = [self initWithColor4B:r2 & 0xffffffff width:r3 height:r4];
    return r0;
}

-(void *)initWithColor3B:(struct _ccColorThreeB)arg2 {
    memcpy(&r2, &arg2, 0x4);
    r0 = [self initWithColor3B:r2 & 0xffffffff width:r3 height:r4];
    return r0;
}

-(void *)initWithColor3B:(struct _ccColorThreeB)arg2 width:(float)arg3 height:(float)arg4 {
    memcpy(&r2, &arg2, 0x4);
    r0 = self;
    r8 = 0xffffffffff000000;
    asm { bfxil      x8, x2, #0x0, #0x18 };
    r0 = [r0 initWithColor4B:r8 width:r3 height:r4];
    return r0;
}

-(void *)initWithColor4B:(struct _ccColor4B)arg2 width:(float)arg3 height:(float)arg4 {
    memcpy(&r2, &arg2, 0x4);
    r31 = r31 - 0x60;
    var_20 = d9;
    stack[-40] = d8;
    var_10 = r20;
    stack[-24] = r19;
    saved_fp = r29;
    stack[-8] = r30;
    r20 = r2;
    r0 = [[&var_30 super] init];
    r19 = r0;
    if (r0 != 0x0) {
            r19->_blendFunc = *0x100ba36a8;
            r8 = sign_extend_64(*(int32_t *)ivar_offset(_realColor));
            *(int8_t *)(r19 + r8) = r20;
            r12 = sign_extend_64(*(int32_t *)ivar_offset(_displayedColor));
            *(int8_t *)(r19 + r12) = r20;
            *(int8_t *)(0x1 + r19 + r8) = r20 >> 0x8;
            *(int8_t *)(0x1 + r19 + r12) = r20 >> 0x8;
            *(int8_t *)(0x2 + r19 + r8) = r20 >> 0x10;
            *(int8_t *)(0x2 + r19 + r12) = r20 >> 0x10;
            *(int8_t *)(int64_t *)&r19->_realOpacity = r20 >> 0x18;
            *(int8_t *)(int64_t *)&r19->_displayedOpacity = r20 >> 0x18;
            r8 = sign_extend_64(*(int32_t *)ivar_offset(_squareVertices));
            *(int128_t *)(r19 + r8) = q0;
            *(int128_t *)(0x10 + r19 + r8) = q0;
            [r19 updateColor];
            [r19 updateOpacity];
            asm { fcvt       d0, s9 };
            asm { fcvt       d1, s8 };
            [r19 setContentSize:r2];
            if ([[CCConfiguration sharedConfiguration] useNewRenderer] != 0x0) {
                    sub_100292530();
                    sub_10035c00c(&var_48, [@"ShaderPositionColor" UTF8String]);
                    r0 = sub_10037ec20(r20, &var_48);
                    *((int64_t *)&r19->_mcRenderJob + 0x140) = r0;
                    if ((sign_extend_64(var_31) & 0xffffffff80000000) != 0x0) {
                            operator delete(var_48);
                    }
                    *(int8_t *)(int64_t *)&r19->_mcRenderJobIsDirty = 0x1;
            }
            else {
                    [r19 setShaderProgram:[[CCShaderCache sharedShaderCache] programForKey:@"ShaderPositionColor"]];
            }
    }
    r0 = r19;
    return r0;
}

-(void)setContentSize:(struct CGSize)arg2 {
    memcpy(&r2, &arg2, 0x8);
    r0 = self;
    r31 = r31 - 0x20;
    asm { fcvt       s2, d0 };
    r8 = sign_extend_64(*(int32_t *)ivar_offset(_squareVertices));
    r8 = r0 + r8;
    *(int32_t *)(r8 + 0x8) = s2;
    asm { fcvt       s3, d1 };
    *(int128_t *)(r8 + 0x14) = s3;
    *(int128_t *)(r8 + 0x18) = s2;
    *(int32_t *)(r8 + 0x1c) = s3;
    *(int8_t *)(int64_t *)&r0->_mcRenderJobIsDirty = 0x1;
    [[&var_10 super] setContentSize:r2];
    return;
}

-(void)setOpacity:(unsigned char)arg2 {
    [[&var_20 super] setOpacity:arg2];
    [self updateOpacity];
    return;
}

-(void)setColor:(struct _ccColorThreeB)arg2 {
    memcpy(&r2, &arg2, 0x4);
    [[&var_20 super] setColor:r2 & 0xffffffff];
    [self updateColor];
    return;
}

-(void)updateColor {
    r8 = 0x0;
    r0 = *(int8_t *)(self + sign_extend_64(*(int32_t *)ivar_offset(_displayedColor)));
    asm { ucvtf      s0, s0 };
    asm { fdiv       s0, s0, s2 };
    r9 = sign_extend_64(*(int32_t *)ivar_offset(_squareColors));
    asm { ucvtf      s1, s1 };
    asm { fdiv       s1, s1, s2 };
    asm { ucvtf      s3, s3 };
    r9 = r0 + r9;
    asm { fdiv       s2, s3, s2 };
    do {
            *(int128_t *)(r9 + r8) = s0;
            *(int128_t *)(0x4 + r9 + r8) = s1;
            *(int32_t *)(0x8 + r9 + r8) = 0x437f0000;
            r8 = r8 + 0x10;
    } while (r8 != 0x40);
    *(int8_t *)(int64_t *)&r0->_mcRenderJobIsDirty = 0x1;
    return;
}

-(void)updateOpacity {
    r0 = self;
    r8 = 0x0;
    r9 = sign_extend_64(*(int32_t *)ivar_offset(_displayedOpacity));
    r10 = sign_extend_64(*(int32_t *)ivar_offset(_squareColors));
    r10 = 0xc + r10 + r0;
    do {
            r0 = *(int8_t *)(r0 + r9);
            asm { ucvtf      s0, s0 };
            asm { fdiv       s0, s0, s1 };
            *(int32_t *)(r10 + r8) = s0;
            r8 = r8 + 0x10;
    } while (r8 != 0x40);
    *(int8_t *)(int64_t *)&r0->_mcRenderJobIsDirty = 0x1;
    return;
}

-(void)updateDisplayedColor:(struct _ccColorThreeB)arg2 {
    memcpy(&r2, &arg2, 0x4);
    [[&var_20 super] updateDisplayedColor:r2 & 0xffffffff];
    [self updateColor];
    return;
}

-(void)updateDisplayedOpacity:(unsigned char)arg2 {
    [[&var_20 super] updateDisplayedOpacity:arg2];
    [self updateOpacity];
    return;
}

-(void)dealloc {
    [[&var_10 super] dealloc];
    return;
}

-(struct _ccBlendFunc)blendFunc {
    r0 = self->_blendFunc;
    return r0;
}

-(void)setBlendFunc:(struct _ccBlendFunc)arg2 {
    memcpy(&r2, &arg2, 0x8);
    self->_blendFunc = r2;
    return;
}

-(void)draw {
    var_50 = r28;
    stack[-88] = r27;
    var_40 = r26;
    stack[-72] = r25;
    var_30 = r24;
    stack[-56] = r23;
    var_20 = r22;
    stack[-40] = r21;
    var_10 = r20;
    stack[-24] = r19;
    saved_fp = r29;
    stack[-8] = r30;
    r29 = &saved_fp;
    r31 = r31 + 0xffffffffffffffa0 - 0x230;
    r19 = self;
    if ([[CCConfiguration sharedConfiguration] useNewRenderer] == 0x0) goto loc_10035c670;

loc_10035c36c:
    r10 = sign_extend_64(*(int32_t *)ivar_offset(_mcRenderJobIsDirty));
    if (*(int8_t *)(r19 + r10) == 0x0) goto loc_10035c7f0;

loc_10035c384:
    r23 = 0x0;
    r8 = 0x0;
    r9 = 0x0;
    r28 = 0x0;
    *(int8_t *)(r19 + r10) = 0x0;
    r11 = r29 - 0xe8;
    var_60 = 0x0;
    *(int128_t *)((r29 - 0xe8) + 0x78) = q0;
    *(int128_t *)((r29 - 0xe8) + 0x68) = q0;
    *(int128_t *)((r29 - 0xe8) + 0x58) = q0;
    *(int128_t *)((r29 - 0xe8) + 0x48) = q0;
    *(int128_t *)((r29 - 0xe8) + 0x38) = q0;
    *(int128_t *)((r29 - 0xe8) + 0x28) = q0;
    *(int128_t *)((r29 - 0xe8) + 0x18) = q0;
    *(int128_t *)((r29 - 0xe8) + 0x8) = q0;
    var_270 = r11 + 0x8;
    var_268 = (int64_t *)&r19->_squareVertices;
    var_278 = r11 + 0x30;
    var_280 = r11 + 0x78;
    goto loc_10035c3ec;

loc_10035c3ec:
    r10 = var_268 + r28 * 0x8;
    r27 = *(int128_t *)r10;
    r22 = *(int128_t *)(r10 + 0x4);
    if (r9 >= r8) goto loc_10035c414;

loc_10035c400:
    *(int128_t *)r9 = r27;
    *(int128_t *)(r9 + 0x4) = r22;
    var_B0 = r9 + 0x8;
    r23 = var_68;
    goto loc_10035c4a8;

loc_10035c4a8:
    r8 = sign_extend_64(*(int32_t *)ivar_offset(_displayedColor));
    r27 = *(int8_t *)(r19 + r8);
    r22 = *(int8_t *)(0x1 + r19 + r8);
    r25 = *(int8_t *)(0x2 + r19 + r8);
    r26 = *(int8_t *)(int64_t *)&r19->_displayedOpacity;
    r8 = var_60;
    if (r23 >= r8) goto loc_10035c4fc;

loc_10035c4e0:
    *(int8_t *)r23 = r27;
    *(int8_t *)(r23 + 0x1) = r22;
    *(int8_t *)(r23 + 0x2) = r25;
    *(int8_t *)(r23 + 0x3) = r26;
    r26 = r23 + 0x4;
    var_68 = r26;
    goto loc_10035c5b8;

loc_10035c5b8:
    r9 = var_D8;
    r8 = var_D0;
    if (r9 >= r8) goto loc_10035c5d0;

loc_10035c5c4:
    *(int16_t *)r9 = r28;
    r9 = r9 + 0x2;
    var_D8 = r9;
    goto loc_10035c658;

loc_10035c658:
    r28 = r28 + 0x1;
    if (r28 == 0x4) goto loc_10035c744;

loc_10035c664:
    r9 = var_B0;
    r8 = var_A8;
    r23 = r26;
    goto loc_10035c3ec;

loc_10035c744:
    sub_100382ea4((int64_t *)&r19->_mcRenderJob + 0x40, r29 - 0xe8);
    r0 = var_70;
    if (r0 != 0x0) {
            operator delete(r0);
    }
    r0 = var_88;
    if (r0 != 0x0) {
            operator delete(r0);
    }
    r0 = var_A0;
    if (r0 != 0x0) {
            operator delete(r0);
    }
    r0 = var_B8;
    if (r0 != 0x0) {
            operator delete(r0);
    }
    r20 = var_C0;
    if (r20 != 0x0) {
            do {
                    asm { ldaxr      x9, [x8] };
                    r10 = r9 - 0x1;
                    asm { stlxr      w11, x10, [x8] };
            } while (r11 != 0x0);
            if (r9 == 0x0) {
                    (*(*r20 + 0x10))(r20);
                    std::__1::__shared_weak_count::__release_weak();
            }
    }
    r0 = var_E0;
    if (r0 != 0x0) {
            operator delete(r0);
    }
    goto loc_10035c7f0;

loc_10035c7f0:
    r0 = sub_1003812ec();
    r19 = (int64_t *)&r19->_mcRenderJob;
    *(int32_t *)(r19 + 0x18) = r0;
    *(int32_t *)(r19 + 0x1c) = sub_1003812ec();
    r8 = &var_260;
    *(r8 + 0x128) = *(r19 + 0x128);
    r9 = *(r19 + 0x130);
    var_130 = r9;
    if (r9 != 0x0) {
            do {
                    asm { ldxr       x10, [x9] };
                    r10 = r10 + 0x1;
                    asm { stxr       w11, x10, [x9] };
            } while (r11 != 0x0);
    }
    r9 = *(int32_t *)(r19 + 0x138);
    r1 = r19 + 0x150;
    sub_1002c5700(r8 + 0x150, r1);
    sub_1003805bc(&var_260);
    sub_1002c59fc(&var_260 + 0x150);
    r19 = var_130;
    if (r19 != 0x0) {
            do {
                    asm { ldaxr      x9, [x8] };
                    asm { stlxr      w11, x10, [x8] };
            } while (r11 != 0x0);
            if (r9 == 0x0) {
                    (*(*r19 + 0x10))(r19);
                    std::__1::__shared_weak_count::__release_weak();
            }
    }
    loc_100381804(&var_260 + 0xe8, r1);
    loc_100381804(&var_260 + 0xb0, r1);
    loc_100381804(&var_260 + 0x78, r1);
    loc_100381804(&var_260 + 0x40, r1);
    return;

loc_10035c5d0:
    r23 = var_E0;
    r24 = r9 - r23;
    if (r24 <= -0x3) goto loc_10035c984;

loc_10035c5e0:
    r21 = SAR(r24, 0x1);
    r9 = r21 + 0x1;
    r8 = r8 - r23;
    if (r8 < r9) {
            if (!CPU_FLAGS & B) {
                    r9 = r8;
            }
            else {
                    r9 = r9;
            }
    }
    if (0x3fffffffffffffff > r8 / 0x2) {
            if (!CPU_FLAGS & A) {
                    r20 = 0x7fffffffffffffff;
            }
            else {
                    r20 = r9;
            }
    }
    if (r20 == 0x0) goto loc_10035c61c;

loc_10035c608:
    if ((r20 & 0xffffffff80000000) != 0x0) goto loc_10035c9a0;

loc_10035c60c:
    r25 = operator new();
    goto loc_10035c620;

loc_10035c620:
    r20 = r25 + r20 * 0x2;
    *(int16_t *)(r25 + r21 * 0x2) = r28;
    r21 = 0x2 + r25 + r21 * 0x2;
    if (r24 >= 0x1) {
            memcpy(r25, r23, r24);
    }
    r10 = 0x3fffffffffffffff;
    var_E0 = r25;
    var_D8 = r21;
    var_D0 = r20;
    if (r23 != 0x0) {
            r10 = 0x3fffffffffffffff;
            operator delete(r23);
    }
    goto loc_10035c658;

loc_10035c9a0:
    sub_10035cb08();
    return;

loc_10035c61c:
    r25 = 0x0;
    goto loc_10035c620;

loc_10035c984:
    std::__1::__vector_base_common<true>::__throw_length_error();
    return;

loc_10035c4fc:
    r24 = var_70;
    r21 = SAR(r23 - r24, 0x2);
    r9 = r21 + 0x1;
    if (r9 >> 0x3e != 0x0) goto loc_10035c978;

loc_10035c514:
    r8 = r8 - r24;
    r10 = SAR(r8, 0x1);
    if (r10 < r9) {
            if (!CPU_FLAGS & B) {
                    r9 = r10;
            }
            else {
                    r9 = r9;
            }
    }
    if (0x1fffffffffffffff > r8 / 0x4) {
            if (!CPU_FLAGS & A) {
                    r20 = 0x3fffffffffffffff;
            }
            else {
                    r20 = r9;
            }
    }
    if (r20 == 0x0) goto loc_10035c54c;

loc_10035c538:
    if (r20 >> 0x3e != 0x0) goto loc_10035c998;

loc_10035c540:
    r0 = operator new();
    goto loc_10035c550;

loc_10035c550:
    r10 = 0x1fffffffffffffff;
    r8 = r0 + r21 * 0x4;
    *(int8_t *)r8 = r27;
    *(int8_t *)(r8 + 0x1) = r22;
    r9 = r0 + r20 * 0x4;
    *(int8_t *)(r8 + 0x2) = r25;
    *(int8_t *)(r8 + 0x3) = r26;
    r26 = r8 + 0x4;
    if (r23 != r24) {
            do {
                    r23 = r23 + 0xfffffffffffffffc;
                    *(int8_t *)(r8 + 0xfffffffffffffffc) = *(int8_t *)r23;
                    *(int8_t *)(r8 + 0xfffffffffffffffd) = *(int8_t *)(r23 + 0x1);
                    *(int8_t *)(r8 + 0xfffffffffffffffe) = *(int8_t *)(r23 + 0x2);
                    r10 = *(int8_t *)(r23 + 0x3);
                    *(int8_t *)(r8 + 0xffffffffffffffff) = r10;
                    r8 = r8 - 0x4;
            } while (r24 != r23);
            r23 = var_70;
    }
    var_70 = r8;
    var_68 = r26;
    var_60 = r9;
    if (r23 != 0x0) {
            operator delete(r23);
    }
    goto loc_10035c5b8;

loc_10035c998:
    sub_10035cb08();
    return;

loc_10035c54c:
    r0 = 0x0;
    goto loc_10035c550;

loc_10035c978:
    std::__1::__vector_base_common<true>::__throw_length_error();
    return;

loc_10035c414:
    r24 = var_B8;
    r25 = r9 - r24;
    r21 = SAR(r25, 0x3);
    r9 = r21 + 0x1;
    if (r9 >> 0x3d != 0x0) goto loc_10035c96c;

loc_10035c42c:
    r8 = r8 - r24;
    r10 = SAR(r8, 0x2);
    if (r10 < r9) {
            if (!CPU_FLAGS & B) {
                    r9 = r10;
            }
            else {
                    r9 = r9;
            }
    }
    if (0xfffffffffffffff > r8 / 0x8) {
            if (!CPU_FLAGS & A) {
                    r20 = 0x1fffffffffffffff;
            }
            else {
                    r20 = r9;
            }
    }
    if (r20 == 0x0) goto loc_10035c468;

loc_10035c450:
    if (r20 >> 0x3d != 0x0) goto loc_10035c990;

loc_10035c458:
    r26 = operator new();
    goto loc_10035c46c;

loc_10035c46c:
    r20 = r26 + r20 * 0x8;
    *(int128_t *)(r26 + r21 * 0x8) = r27;
    *(int128_t *)(0x4 + r26 + r21 * 0x8) = r22;
    r21 = 0x8 + r26 + r21 * 0x8;
    if (r25 >= 0x1) {
            memcpy(r26, r24, r25);
    }
    r10 = 0xfffffffffffffff;
    var_B8 = r26;
    var_B0 = r21;
    var_A8 = r20;
    if (r24 != 0x0) {
            r10 = 0xfffffffffffffff;
            operator delete(r24);
    }
    goto loc_10035c4a8;

loc_10035c990:
    sub_10035cb08();
    return;

loc_10035c468:
    r26 = 0x0;
    goto loc_10035c46c;

loc_10035c96c:
    std::__1::__vector_base_common<true>::__throw_length_error();
    return;

loc_10035c670:
    sub_10031a6c4();
    r20 = sign_extend_64(*(int32_t *)ivar_offset(_shaderProgram));
    [*(r19 + r20) use];
    [*(r19 + r20) setUniformsForBuiltins];
    sub_10031a6f0(0x3);
    glDisableVertexAttribArray(0x2);
    glVertexAttribPointer(0x0, 0x2, 0x1406, 0x0, 0x0, (int64_t *)&r19->_squareVertices);
    glVertexAttribPointer(0x1, 0x4, 0x1406, 0x0, 0x0, (int64_t *)&r19->_squareColors);
    r8 = sign_extend_64(*(int32_t *)ivar_offset(_blendFunc));
    sub_10031a4a8(*(int128_t *)(r19 + r8), *(int128_t *)(0x4 + r19 + r8));
    glDrawArrays(0x5, 0x0, 0x4);
    *0x1011e2dd0 = *0x1011e2dd0 + 0x1;
    return;
}

@end